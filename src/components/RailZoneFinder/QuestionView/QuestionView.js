...
import { FormContext, MapContext } from 'globalState';
...
const QuestionView = () => {
  ...
  const changeView = (listView) => {
    formDispatch({ type: 'UPDATE_VIEW', payload: false });
    if (listView) {
      mapDispatch({ type: 'UPDATE_VIEW', payload: false });
    }
    window.scrollTo(0, 0);
  };
